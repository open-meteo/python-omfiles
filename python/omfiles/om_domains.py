...
class OmDomain:
    ...
    def __init__(self, name: str, grid: AbstractGrid, file_length: int, temporal_resolution_seconds: int = 3600):
        """
...
_dwd_icon_grid = RegularLatLonGrid(
    lat_start=-90, lat_steps=1441, lat_step_size=0.125, lon_start=-180, lon_steps=2879, lon_step_size=0.125
)
...
_dwd_icon_eu_grid = RegularLatLonGrid(
    lat_start=29.5, lat_steps=657, lat_step_size=0.0625, lon_start=-23.5, lon_steps=1377, lon_step_size=0.0625
)
...
_dwd_icon_d2_grid = RegularLatLonGrid(
    lat_start=43.18, lat_steps=746, lat_step_size=0.02, lon_start=-3.94, lon_steps=1215, lon_step_size=0.02
)
...
_dwd_icon_eps_grid = RegularLatLonGrid(
    lat_start=-90, lat_steps=721, lat_step_size=0.25, lon_start=-180, lon_steps=1439, lon_step_size=0.25
)
...
_dwd_icon_eu_eps_grid = RegularLatLonGrid(
    lat_start=29.5, lat_steps=329, lat_step_size=0.125, lon_start=-23.5, lon_steps=689, lon_step_size=0.125
)
...
_dwd_icon_d2_eps_grid = RegularLatLonGrid(
    lat_start=43.18, lat_steps=745, lat_step_size=0.02, lon_start=-3.94, lon_steps=1214, lon_step_size=0.02
)
...
_meteofrance_arpege_europe_grid = RegularLatLonGrid(
    lat_start=20, lat_steps=521, lat_step_size=0.1, lon_start=-32, lon_steps=741, lon_step_size=0.1
)
...
_meteofrance_arpege_world025_grid = RegularLatLonGrid(
    lat_start=-90, lat_steps=721, lat_step_size=0.25, lon_start=-180, lon_steps=1440, lon_step_size=0.25
)
...
_meteofrance_arome_france0025_grid = RegularLatLonGrid(
    lat_start=37.5, lat_steps=717, lat_step_size=0.025, lon_start=-12.0, lon_steps=1121, lon_step_size=0.025
)
...
_meteofrance_arome_france_hd_grid = RegularLatLonGrid(
    lat_start=37.5, lat_steps=1791, lat_step_size=0.01, lon_start=-12.0, lon_steps=2801, lon_step_size=0.01
)
...
_gem_global_grid = RegularLatLonGrid(
    lat_start=-90, lat_steps=1201, lat_step_size=0.15, lon_start=-180, lon_steps=2400, lon_step_size=0.15
)
...
_gem_regional_projection = StereographicProjection(latitude=90, longitude=249, radius=6371229)
...
_gem_hrdps_projection = RotatedLatLonProjection(lat_origin=-36.0885, lon_origin=245.305)
...
_gem_global_ensemble_grid = RegularLatLonGrid(
    lat_start=-90, lat_steps=361, lat_step_size=0.5, lon_start=-180, lon_steps=720, lon_step_size=0.5
)
...
DOMAINS: dict[str, OmDomain] = {
    ...
    "ecmwf_ifs025": OmDomain(
        name="ecmwf_ifs025", grid=_ecmwf_ifs025_grid, file_length=104, temporal_resolution_seconds=3600 * 3
    ),
